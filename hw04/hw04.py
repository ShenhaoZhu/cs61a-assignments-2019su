...
def make_withdraw(balance, password):
    ...
    times = 0
    wrong_password = []
    def withdraw(drown, u_pass):
        nonlocal bal, times, wrong_password
        if u_pass != pass_ and times < 3:
            times += 1
            wrong_password.append(u_pass)
            return 'Incorrect password'
        elif times >= 3:
            # wrong_password.append(u_pass)
            return "Your account is locked. Attempts: {}".format(wrong_password)
        elif u_pass == pass_ and drown > bal:
